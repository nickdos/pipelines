...
//      t -> "stringArrayContains(" + HiveColumnsUtils.getHiveQueryColumn(t) + ",'%s',%b)";
    ...
    if (predicate.getKey().name().equals("TAXON_KEY")) {
      ...
      Optional.ofNullable(VocabularyUtils.lookupEnum(predicate.getValue(), MediaType.class))
          .ifPresent(
              mediaType ->
                  builder.append(
                      String.format(getArrayFn().apply(GbifTerm.mediaType), mediaType.name(), true)));
      ...
      Optional.ofNullable(VocabularyUtils.lookupEnum(predicate.getValue(), TypeStatus.class))
          .ifPresent(
              typeStatus ->
                  builder.append(
                      String.format(getArrayFn().apply(DwcTerm.typeStatus), typeStatus.name(), true)));
    } else if ("ISSUE".equals(predicate.getKey().name())) {
      builder.append(
          String.format(getArrayFn().apply(GbifTerm.issue), predicate.getValue().toUpperCase(), true));
  ...
  public void visit(InPredicate predicate) throws QueryBuildingException {
    ...
    if (isSQLArray(predicate.getKey()) || TermUtils.isVocabulary(term(predicate.getKey()))) {
  ...
  public void visit(IsNotNullPredicate predicate) throws QueryBuildingException {
    if (isSQLArray(predicate.getParameter())
        || TermUtils.isVocabulary(term(predicate.getParameter()))) {
      builder.append(
