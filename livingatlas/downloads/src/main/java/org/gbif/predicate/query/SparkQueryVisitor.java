...
public abstract class SparkQueryVisitor implements QueryVisitor {
            ...
            term -> {
              if (term instanceof DwcTerm) {
                ...
                if (String.class.isAssignableFrom(param.type())
                    && OccurrenceSearchParameter.GEOMETRY != param
                    && !matchCase) {
  ...
  /**
   * FIXME - this can probably be factored out once all required fields are denormalized.
   *
   * @param param
   * @param matchCase
   * @return
   ...
   * a strings that can be used as the <em>WHERE</em> clause for a Hive download.
   *
  ...
  public void visit(org.gbif.api.model.predicate.ConjunctionPredicate predicate)
      throws QueryBuildingException {
    visitCompoundPredicate(predicate, CONJUNCTION_OPERATOR);
  }

  public void visit(org.gbif.api.model.predicate.DisjunctionPredicate predicate)
      throws QueryBuildingException {
    ...
    for (Predicate subPredicate : predicate.getPredicates()) {
      if (subPredicate instanceof org.gbif.api.model.predicate.EqualsPredicate) {
        org.gbif.api.model.predicate.EqualsPredicate equalsSubPredicate =
            (org.gbif.api.model.predicate.EqualsPredicate) subPredicate;
        if (parameter == null) {
  ...
  public void visit(org.gbif.api.model.predicate.EqualsPredicate predicate)
      throws QueryBuildingException {
    ...
    // MediaType.class))
    //                    .ifPresent(mediaType ->
    // builder.append(String.format(ARRAY_FN.apply(GbifTerm.mediaType), mediaType.name(), true)));
    //        } else if (OccurrenceSearchParameter.TYPE_STATUS == predicate.getKey()) {
    //            Optional.ofNullable(VocabularyUtils.lookupEnum(predicate.getValue(),
    // TypeStatus.class))
    //                    .ifPresent(typeStatus ->
    // builder.append(String.format(ARRAY_FN.apply(DwcTerm.typeStatus), typeStatus.name(), true)));
    //        } else if (OccurrenceSearchParameter.ISSUE == predicate.getKey()) {
    //            builder.append(String.format(ARRAY_FN.apply(GbifTerm.issue),
    // predicate.getValue().toUpperCase(), true));
    //        } else
    if (getArrayTerms().containsKey(predicate.getKey())) {
      builder.append(
          String.format(
              ARRAY_FN.apply(getArrayTerms().get(predicate.getKey())),
              predicate.getValue(),
              predicate.isMatchCase()));
    } else if (TermUtils.isVocabulary(term(predicate.getKey()))) {
      builder.append(
          String.format(ARRAY_FN.apply(term(predicate.getKey())), predicate.getValue(), true));
  ...
  public void visit(org.gbif.api.model.predicate.GreaterThanOrEqualsPredicate predicate)
      throws QueryBuildingException {
  ...
  public void visit(org.gbif.api.model.predicate.GreaterThanPredicate predicate)
      throws QueryBuildingException {
    if (Date.class.isAssignableFrom(predicate.getKey().type())) {
  ...
  public void visit(org.gbif.api.model.predicate.LessThanOrEqualsPredicate predicate)
      throws QueryBuildingException {
    if (Date.class.isAssignableFrom(predicate.getKey().type())) {
      // Where the date is a range, consider the "OrEquals" to mean including the whole range.
  ...
  public void visit(org.gbif.api.model.predicate.LessThanPredicate predicate)
      throws QueryBuildingException {
    if (Date.class.isAssignableFrom(predicate.getKey().type())) {
      // Where the date is a range, consider the lack of "OrEquals" to mean excluding the whole
      // range.
   ...
   */
  public void visit(org.gbif.api.model.predicate.InPredicate predicate)
      throws QueryBuildingException {
    ...
    if (isHiveArray(predicate.getKey()) || TermUtils.isVocabulary(term(predicate.getKey()))) {
      ...
      while (iterator.hasNext()) {
        ...
        visit(
            new org.gbif.api.model.predicate.EqualsPredicate(
                predicate.getKey(), iterator.next(), isMatchCase));
        if (iterator.hasNext()) {
  ...
  public void visit(org.gbif.api.model.predicate.LikePredicate predicate)
      throws QueryBuildingException {
    // Replace % → \% and _ → \_
    // Then replace * → % and ? → _
    org.gbif.api.model.predicate.LikePredicate likePredicate =
        new org.gbif.api.model.predicate.LikePredicate(
            predicate.getKey(),
            predicate
                .getValue()
                .replace("%", "\\%")
  ...
  public void visit(org.gbif.api.model.predicate.NotPredicate predicate)
      throws QueryBuildingException {
    builder.append(NOT_OPERATOR);
    visit(predicate.getPredicate());
  }

  public void visit(org.gbif.api.model.predicate.IsNotNullPredicate predicate)
      throws QueryBuildingException {
    builder.append(toSparkSQLField(predicate.getParameter(), true));
    builder.append(IS_NOT_NULL_OPERATOR);
  }

  public void visit(org.gbif.api.model.predicate.IsNullPredicate predicate)
      throws QueryBuildingException {
    builder.append(toSparkSQLField(predicate.getParameter(), true));
   ...
   * </pre>
   */
  public void visitCompoundPredicate(
      org.gbif.api.model.predicate.CompoundPredicate predicate, String op)
      throws QueryBuildingException {
    builder.append('(');
  ...
  public void visitSimplePredicate(
      org.gbif.api.model.predicate.SimplePredicate predicate, String op)
      throws QueryBuildingException {
  ...
  public void visitSimplePredicate(
      org.gbif.api.model.predicate.SimplePredicate predicate, String op, String value) {
    builder.append(toSparkSQLField(predicate.getKey(), predicate.isMatchCase()));
    builder.append(op);
  ...
  private static Predicate toNumberRangePredicate(Range<Double> range, SearchParameter key) {
    if (!range.hasLowerBound()) {
      return new org.gbif.api.model.predicate.LessThanOrEqualsPredicate(
          key, String.valueOf(range.upperEndpoint().doubleValue()));
    }
    if (!range.hasUpperBound()) {
      return new org.gbif.api.model.predicate.GreaterThanOrEqualsPredicate(
    ...
    ImmutableList<Predicate> predicates =
        new ImmutableList.Builder<Predicate>()
            .add(
                new org.gbif.api.model.predicate.GreaterThanOrEqualsPredicate(
                    key, String.valueOf(range.lowerEndpoint().doubleValue())))
            .add(
                new org.gbif.api.model.predicate.LessThanOrEqualsPredicate(
                    key, String.valueOf(range.upperEndpoint().doubleValue())))
            .build();
